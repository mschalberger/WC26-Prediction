...
# Install: install.packages(c("shiny", "dplyr", "DT", "shinycssloaders"))
...
simulate_match <- function(elo_home, elo_away, k = 20, neutral = FALSE) {
    p_h <- elo_expected(elo_home, elo_away)
    draw_p <- 1/3*exp(-((p_h-.5)^2)/(2*0.28^2))#pmin(0.28 * 4 * p_h * (1 - p_h), 0.30)
    ph <- p_h * (1 - draw_p); pa <- (1 - p_h) * (1 - draw_p)
    outcome <- sample(c("home","draw","away"), 1, prob = c(ph, draw_p, pa))
    lh <- 1.8*ph+0.27#pmax(pmin(exp(0.4 + 0.0004*(elo_home - elo_away)), 4), 0.3)
    la <- 1.8*pa+0.27#pmax(pmin(exp(0.4 - 0.0004*(elo_home - elo_away)), 4), 0.3)
    repeat {
...
run_group_stage <- function(teams_df) {
    ...
    for (grp in sort(unique(teams_df$group_letter))) {
        ...
        for (pair in pairs) {
            ...
            else { pts[as.character(h)] <- pts[as.character(h)]+1; pts[as.character(a)] <- pts[as.character(a)]+1 }
        ...
        standing <- data.frame(id=ids, pts=as.numeric(pts), gf=as.numeric(gf), ga=as.numeric(ga)) %>%
...
sim_ko_match <- function(id_a, id_b, elo_live, teams_df, round_name, k=20) {
    res <- simulate_match(elo_live[as.character(id_a)], elo_live[as.character(id_b)], k=k, neutral=TRUE)
    pens <- ""
    if (res$outcome=="draw") {
...
run_knockout <- function(pairs, elo_live, teams_df, round_name) {
    winners <- c(); losers <- c(); rows <- data.frame()
    for (pair in pairs) {
        res <- sim_ko_match(pair[1], pair[2], elo_live, teams_df, round_name)
...
run_tournament <- function(seed=NULL) {
    if (!is.null(seed)) set.seed(seed)
    teams_df <- teams_init

    # Group Stage
    gs <- run_group_stage(teams_df)
    elo_live <- gs$elo_live
    std <- gs$standings

    # 8 best 3rd-place teams
    thirds <- std %>% filter(rank==3) %>% arrange(desc(pts),desc(gd),desc(gf)) %>% slice(1:8)

    get_t <- function(grp,rnk) std %>% filter(group==grp, rank==rnk) %>% pull(id)

    # R32 bracket (official WC2026 paths approximated)
    r32_pairs <- list(
    ...
    r32 <- run_knockout(r32_pairs, elo_live, teams_df, "Round of 32")
    elo_live <- r32$elo_live
    r16_pairs <- lapply(seq(1,15,2), function(i) c(r32$winners[i], r32$winners[i+1]))
    r16 <- run_knockout(r16_pairs, elo_live, teams_df, "Round of 16")
    elo_live <- r16$elo_live
    qf_pairs <- lapply(seq(1,7,2), function(i) c(r16$winners[i], r16$winners[i+1]))
    qf <- run_knockout(qf_pairs, elo_live, teams_df, "Quarter-Final")
    elo_live <- qf$elo_live
    sf_pairs <- list(c(qf$winners[1],qf$winners[2]), c(qf$winners[3],qf$winners[4]))
    sf <- run_knockout(sf_pairs, elo_live, teams_df, "Semi-Final")
    elo_live <- sf$elo_live
    sf_losers <- sf$losers
    tp <- run_knockout(list(sf_losers), elo_live, teams_df, "Third Place")
    elo_live <- tp$elo_live
    fin <- run_knockout(list(c(sf$winners[1],sf$winners[2])), elo_live, teams_df, "Final")
    ...
    all_ko_matches <- bind_rows(r32$matches, r16$matches, qf$matches, sf$matches, tp$matches, fin$matches)

    final_elo <- data.frame(id=as.integer(names(elo_live)), final_elo=round(as.numeric(elo_live))) %>%
        left_join(teams_df %>% select(id,team_name,fifa_code,elo), by="id") %>%
        mutate(change=final_elo-elo, flag=sapply(fifa_code, get_flag)) %>%
        arrange(desc(final_elo)) %>%
        mutate(rank=row_number())

    list(standings=std, group_matches=gs$matches, ko_matches=all_ko_matches,
         champion=champion, runner_up=runner, third=third, final_elo=final_elo)
}
...
ui <- fluidPage(
    tags$head(
      ...
      * { box-sizing: border-box; }
      ...
      .control-bar label { color: var(--muted); font-size: 12px; letter-spacing: 1px; text-transform: uppercase; }
      .control-bar .form-group { margin: 0; }
      #seed { background: #1C1C2A; border: 1px solid var(--border); color: var(--text);
              border-radius: 6px; padding: 8px 12px; width: 100px; font-family: monospace; }
      #run_btn {
      ...
      .tab-content { background: var(--panel); border: 1px solid var(--border);
      ...
      .group-table th {
        ...
        text-transform: uppercase; padding: 6px 12px; text-align: right;
        font-weight: 400;
      }
      ...
      .ko-score { font-family: monospace; font-size: 15px; font-weight: 600;
      ...
      .elo-bar { background: linear-gradient(90deg, var(--blue), var(--gold)); height: 6px; border-radius: 3px; }
    ...
    div(class="wc-body",
        ...
        div(class="control-bar",
            ...
            tags$span(id="sim_info", style="color:var(--muted);font-size:12px;margin-left:8px;",
                      "Click to run a full tournament simulation")
        ),
...
server <- function(input, output, session) {
    ...
    # Auto-run on startup
    observe({ result(run_tournament(seed=123)) })

    observeEvent(input$run_btn, {
        seed <- as.integer(input$seed)
        if (is.na(seed)) seed <- sample(1:99999, 1)
        result(run_tournament(seed=seed))
        updateTextInput(session, "sim_info", value="")
    })
        ...
        make_card <- function(team, cls, label, trophy) {
            div(class=paste("podium-card", cls),
                ...
                div(style="color:var(--muted);font-size:11px;margin-top:4px;",
                    paste("ELO:", round(r$final_elo %>% filter(id==team$id) %>% pull(final_elo))))
            )
        }
    ...
    output$groups_ui <- renderUI({
        ...
        cards <- lapply(sort(unique(std$group)), function(grp) {
            ...
            rows <- lapply(1:nrow(gd), function(i) {
                ...
                tags$tr(class=cls,
                        tags$td(
                            tags$span(class=paste("rank-badge", rank_cls), i),
    ...
    output$group_matches_ui <- renderUI({
        ...
        sections <- lapply(sort(unique(gm$group)), function(grp) {
            matches <- gm %>% filter(group==grp)
            rows <- lapply(1:nrow(matches), function(i) {
                m <- matches[i,]
                tags$tr(
                    tags$td(m$home),
                    tags$td(class="ko-score", m$score),
                    tags$td(m$away)
                )
            })
            div(class="ko-section",
                ...
                tags$table(class="ko-table",
                           tags$thead(tags$tr(tags$th("Home"), tags$th(style="text-align:center","Score"), tags$th("Away"))),
                           tags$tbody(rows)
                )
            )
        })
    ...
    output$ko_ui <- renderUI({
        ...
        sections <- lapply(rounds, function(rnd) {
            matches <- ko %>% filter(stage==rnd)
            if (nrow(matches)==0) return(NULL)
            rows <- lapply(1:nrow(matches), function(i) {
                m <- matches[i,]
                final_cls <- if(rnd=="Final") 'style="background:rgba(245,197,24,0.05);"' else ""
                tags$tr(
                    tags$td(m$home),
                    tags$td(class="ko-score", m$score),
                    tags$td(m$away),
                    tags$td(class="ko-winner", m$result)
                )
            })
            div(class="ko-section",
    ...
    output$elo_ui <- renderUI({
        ...
        rows <- lapply(1:nrow(fe), function(i) {
            ...
            tags$tr(
                ...
                tags$td(style="font-family:monospace;color:var(--muted);", row$elo),
                tags$td(
        ...
        tags$table(class="ko-table",
                   tags$thead(tags$tr(
                       ...
                       tags$th("Final ELO"), tags$th("Change"),
                       tags$th("Start ELO"), tags$th("Strength")
                   )),
                   tags$tbody(rows)
        )
    })
